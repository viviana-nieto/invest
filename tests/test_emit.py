...
Validates data.json / exploration.json (per-stock fundamentals + verdict block),
technicals.json (per-ticker signal fields), screen.json, and macro_data.json for
required keys and correct types, so the dashboard can rely on the shape.
"""
...
def test_sparklines_are_nonempty_float_arrays(bundle):
    for o in bundle["data"] + bundle["exploration"]:
        for k in SPARK_KEYS:
            arr = o[k]
...
def test_technicals_complete_per_ticker(bundle):
    ...
    tickers = [o["ticker"] for o in bundle["data"] + bundle["exploration"]]
    for tk in tickers:
...
def test_screen_shape(bundle):
    ...
    assert set(["dip_max", "pbt_cut"]) <= set(s["criteria"])
    assert isinstance(s["universe"], list) and s["universe"]
    for bucket in ("pass", "watch"):
        for e in s[bucket]:
            assert {"ticker", "name", "channel_position", "payback_years",
                    "dip_ok", "pbt_ok"} <= set(e)
...
def test_write_all_produces_every_file(cfg, tmp_path):
    ...
    for f in ["data.json", "exploration.json", "technicals.json", "screen.json", "macro_data.json"]:
